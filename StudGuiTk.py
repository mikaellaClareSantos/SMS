...
class EmpGuiTk(Tk):
    ...
    def __init__(self, dataBase=StudDbSqlite('AppDb.db')):
        ...
        self.class_number_label.place(x=30, y=50)
        self.class_number_entryVar = StringVar()
        self.class_number_entry = self.newCtkEntry(entryVariable=self.class_number_entryVar)
        self.class_number_entry.place(x=110, y=50)
        ...
        self.course_cboxOptions = ['Electronics Engineering', 'Electrical Engineering', 'Computer Engineering', 'Chemical Engineering', 'Mechanical Engineering', 'Others']
        self.course_cbox = self.newCtkComboBox(options=self.course_cboxOptions, 
                                    entryVariable=self.course_cboxVar)
        self.course_cbox.place(x=120, y=180)
        ...
        self.college_department_label.place(x=30, y=290)
    ...
    def clear_form(self, *clicked):
        if clicked:
            ...
            self.tree.focus('')
        self.class_number_entryVar.set('')
    ...
    def read_display_data(self, event):
        ...
        if selected_item:
            ...
            self.clear_form()
            self.class_number_entryVar.set(row[0])
    ...
    def add_entry(self):
        class_number=self.class_number_entryVar.get()
